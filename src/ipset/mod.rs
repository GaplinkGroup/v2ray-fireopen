...
pub fn add_to_ipset(ip_addr: IpAddr, setname: &str) -> Result<(), FetchError> {
    ...
    *nlh.nlmsg_flags = netlink::NLM_F_REQUEST;
    ...
    match ip_addr {
        IpAddr::V4(ipv4) => {
            ...
            debug!("IPv4 '{}' ({}) from wireguard allowips for setname '{}' received.", ipv4, v, setname);
            nlh.put(a_type, &v)
        },
        IpAddr::V6(ipv6) => {
            let v = u128::from(ipv6).to_be();
            debug!("IPv6 '{}' ({}) from wireguard allowips for setname '{}' received.", ipv6, v, setname);
            nlh.put(a_type, &v)
        }
    }?;
...
pub fn del_from_ipset(ip_addr: IpAddr, setname: &str) -> Result<(), FetchError> {
    if (setname.len() + 1) >= IPSET_MAXNAMELEN { // +1 includes C-NUL
        return Err(From::from(format!("'{}' >= {}", setname, IPSET_MAXNAMELEN)));
    }
    ...
    let mut nlh = mnl::Nlmsg::new(&mut buffer)?;
    *nlh.nlmsg_type = IPSET_CMD_DEL | (nfnl::NFNL_SUBSYS_IPSET << 8);
    *nlh.nlmsg_flags = netlink::NLM_F_REQUEST;
